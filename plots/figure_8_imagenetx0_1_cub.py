"""Figure 8 — Fine-tuning after transfer: ImageNet (x0.1 width) → CUB / ResNet-18."""
import glob
import os
...
def figure_8_imagenetx0_1_cub(cfg, outman, prefix, gpu_id):
    output_dir = cfg['output_dir']
    datas = []

    configs = [
        ('transfer_imagenet_x0_1-cub_resnet18_usetarget', 'Oracle Transfer', 'C3', '-.'),
        ('transfer_imagenet_x0_1-cub_resnet18_noperm', 'Naive Transfer', 'C1', '-'),
        ('transfer_imagenet_x0_1-cub_resnet18', 'GMT Transfer', 'C0', '-'),
        ('transfer_imagenet_x0_1-cub_resnet18_fast', 'FGMT Transfer', 'C2', '-'),
    ]
    ...
    if not datas:
        print("  No data available for figure 8 (CUB). Run transfers first.")
        return

    xs = list(range(1, len(datas[0]['means']) + 1))
    name = 'section_4-2_imagenetx0_1-cub_resnet18'
