"""Figure 3 — Synced transfer comparison: CIFAR-10 / Conv8."""
import glob
import os
import torch
...
def figure_3_cifar10_conv8(cfg, outman, prefix, gpu_id):
    output_dir = cfg['output_dir']
    fig, ax = plt.subplots(figsize=(6, 4.8))

    gmt_results = _load_synced_results(output_dir, 'synced_transfer_init-cifar10_conv8')
    if gmt_results:
        r = gmt_results[0]
        ...
        print(f"  WARNING: no results for synced_transfer_init-cifar10_conv8")

    rnd_results = _load_synced_results(output_dir, 'synced_transfer_init-cifar10_conv8_random')
    if rnd_results:
        r = rnd_results[0]
    ...
    name = 'compare_sync_diff_cifar10_conv8_500iters'
